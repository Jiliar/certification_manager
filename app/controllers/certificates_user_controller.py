...
class CertificateUserController:
    ...
    def create_table(self):
        try:
            if not db.engine.dialect.has_table(db.engine.connect(), CertificateUser.__tablename__):
                db.Base.metadata.create_all(db.engine)
                self.logging.info("create_table(): table was created successfully "+CertificateUser.__tablename__)
                db.session.close
            else:
                self.logging.info("create_table(): table "+CertificateUser.__tablename__+" wasn't created, because it already exists")
        except Exception as e:
           self.logging.error("create_table(): {0} - Details: {1}".format(Error.E004.value, str(e)))
    
    
    def insert(self, obj: CertificateUser):
        try:
            db.session.add(obj)
        except Exception as e:
    ...
    def get(self, id):
        res = None
        try:
            res = db.session.query(CertificateUser).get(id)
            db.session.close
        except Exception as e:
    ...
    def list(self):
        res = []
        try:
            res = db.session.query(CertificateUser).all()
            db.session.close
        except Exception as e:
    ...
    def update(self, id, obj: CertificateUser):
        res = None
        try:
    ...
    def delete(self, obj: CertificateUser):
        res = None
        try:
